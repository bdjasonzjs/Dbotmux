...
describe('im.message.receive_v1 — bot-to-bot @mention routing', () => {
  ...
  it('routes an unknown external bot real @mention without a grant card in restricted mode', async () => {
    setupBotState({ allowedUsers: ['ou_owner'] });
  ...
  it('does not open a pending grant when a real bot @mention routes directly', async () => {
    setupBotState({
      allowedUsers: ['ou_owner'],
    ...
    expect(handlers.handleNewTopic).not.toHaveBeenCalled();
    expect(mockReplyMessage).not.toHaveBeenCalled();
    expect(getPendingGrantLimits(MY_APP_ID, 'chat-001', OTHER_BOT_OPEN_ID)).toBeUndefined();
  });
  ...
  it('still routes a real bot @mention outside allowedChatGroups', async () => {
  ...
  it('routes an unknown bot real @mention into an owned native topic', async () => {
  ...
  it('treats oncall as chat-level: relaxes canTalk even when THIS bot is not the one bound', async () => {
    ...
    mockGetBot.mockReturnValue({
      config: {
        ...
        larkAppSecret: 'secret',
        cliId: 'claude-code',
        allowedUsers: ['ou_some_other_human'],
      },
  ...
  it('routes a cold same-deployment sibling real @mention without requiring live peer resolution', async () => {
    ...
    expect(mockReplyMessage).not.toHaveBeenCalled();
    expect(mockWriteFileSync.mock.calls.some(([path]) => String(path).includes('bot-openids-'))).toBe(false);
  });

  it('routes a genuine external bot real @mention without consulting the live resolver', async () => {
    setupBotState({ allowedUsers: ['ou_owner'], autoGrantRequestCards: true });
    mockGetOwnerOpenId.mockReturnValue('ou_owner');
    mockReadFileSync.mockReturnValue('{}');
    ...
    expect(handlers.handleThreadReply).toHaveBeenCalledWith(event, expect.objectContaining({
      scope: 'thread',
      anchor: 'root-real-external-topic',
      larkAppId: MY_APP_ID,
    }));
    expect(handlers.handleNewTopic).not.toHaveBeenCalled();
    expect(mockReplyMessage).not.toHaveBeenCalled();
  ...
  it('routes an unknown external bot real @mention when auto grant cards are disabled', async () => {
  ...
  it('routes repeat real @mentions from the same bot+chat without grant cards', async () => {
    setupBotState({ allowedUsers: ['ou_owner'] });
    mockGetOwnerOpenId.mockReturnValue('ou_owner');
    mockGetChatMode.mockResolvedValue('group');
    mockReadFileSync.mockReturnValue('{}');
  ...
  it('does not attempt grant-card sends for repeated real @mentions', async () => {
    setupBotState({ allowedUsers: ['ou_owner'] });
    mockGetOwnerOpenId.mockReturnValue('ou_owner');
    mockGetChatMode.mockResolvedValue('group');
    mockReadFileSync.mockReturnValue('{}');
  ...
  it('routes sender_type="bot" real @mentions from unknown peers', async () => {
  ...
  it('lets an unknown-peer bot real @mention use /t to seed a fresh topic', async () => {
  ...
  it('lets an unknown-peer bot real @mention use /t independently of an owned chat session', async () => {
  ...
  it('lets an unknown-peer bot real @mention use the /topic alias', async () => {
  ...
  it('routes a real @mention in another chat independently of per-chat grants', async () => {
  ...
  it('treats sibling oncall bindings as chat-level talk access', async () => {
    // A shared oncall workspace is chat-scoped: once any sibling is bound,
    // every bot in that chat accepts ordinary talk while operate permission
    // remains independently restricted.
    ...
    expect(handlers.handleThreadReply).not.toHaveBeenCalled();
    expect(canTalk(MY_APP_ID, 'chat-oncall-sibling', USER_OPEN_ID)).toBe(true);
...
describe('im.message.receive_v1 — 急急如律令 summon routing', () => {
  ...
  beforeEach(() => {
    ...
    mockGetChatMode.mockResolvedValue('group');
    mockReadFileSync.mockReturnValue('[]');
    mockExpandLetters = (text: string) => text;
    startLarkEventDispatcher(MY_APP_ID, 'secret', handlers);
  });
  ...
  it('ignores a summon from a non-allowed sender (canTalk gate)', async () => {
    setupClaudeBot({ allowed: ['ou_someone_else'] });
    const event = makeUserSummon('急急如律令：【克劳德】报个数');
    await capturedHandlers['im.message.receive_v1'](event);
    await flushEventWork();
    expect(handlers.handleNewTopic).not.toHaveBeenCalled();
  ...
  it('miss-path: unreadable letter leaves manual-read hint, never a bare sentinel', async () => {
    mockExpandLetters = (text: string) => text.replace(
      /⟪letter:lt_gone⟫/,
      '（信箱取信失败 lt_gone，请手动运行 botmux mailbox read lt_gone 取全文）',
    );
