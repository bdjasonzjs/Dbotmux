...
describe('codex buildArgs', () => {
  ...
  it('resume-fallback reads history.jsonl from cliHome (clone CODEX_HOME)', () => {
    ...
    try {
      ...
      expect(args.at(-1)).toBe('cdx-uuid-1');
      expect(args).toContain('--dangerously-bypass-approvals-and-sandbox');
      expect(args).toContain('shell_environment_policy.set.BOTMUX_SESSION_ID="sess-4"');
...
describe('readyPattern', () => {
  ...
  it('coco matches status bar indicator', () => {
    ...
    expect(adapter.readyPattern!.test('GPT-5.5 · Context 100% left · /tmp')).toBe(true);
    expect(adapter.readyPattern!.test('\n❯ Implement {feature}')).toBe(true);
    expect(adapter.readyPattern!.test('\n❯ 1. Yes, continue')).toBe(false);
  });
