...
import { ensureCommandGuardShims, resolveWorkerSecurityEnv } from '../src/security/worker-security.js';
import {
...
describe('buildBotmuxEnvAssignments()', () => {
  it('forwards only the daemon-side keys; bare LARK_APP_* are NOT forwarded', () => {
    const out = buildBotmuxEnvAssignments({
      ...
      all_proxy: 'http://restricted-proxy:7890',
      ftp_proxy: 'http://restricted-proxy:7890',
      NO_PROXY: 'localhost,scm.corp.invalid',
      no_proxy: 'localhost,scm.corp.invalid',
...
describe('shell wrapper end-to-end (the contract spawn() builds)', () => {
  ...
  it.skipIf(!hasEnvBin)(
    ...
    () => {
      ...
      const guardedScript = shellWrapperScript('/home/u/.botmux/bin', 'sh', guardDir);
  ...
  it.skipIf(!hasEnvBin)(
    'relative guard config stays absolute after the tmux wrapper changes to the worker cwd',
    () => {
      tmpDir = mkdtempSync(join(tmpdir(), 'bmx-guard-relative-wrapper-'));
      const daemonCwd = join(tmpDir, 'daemon');
      const workerCwd = join(tmpDir, 'worker');
      ...
      try {
        ...
        guardDir = ensureCommandGuardShims(guardEnv)!;
        securityPathPrefix = resolveWorkerSecurityEnv(guardEnv).BOTMUX_PATH_PREFIX;
      } finally {
        process.chdir(originalCwd);
      }

      expect(guardDir!).toBe(join(daemonCwd, 'guard'));
      expect(securityPathPrefix!).toBe(guardDir!);
      const guardedScript = shellWrapperScript('/home/u/.botmux/bin', 'sh', securityPathPrefix!);
      const result = spawnSync(
        '/bin/sh',
        ['-c', guardedScript, '_', workerCwd, 'BOTMUX_COMMAND_GUARD=1',
          '/bin/sh', '-c', 'command -v node; node'],
        { encoding: 'utf-8', env: { PATH: '/usr/bin:/bin' } },
      );
      expect(result.status).toBe(0);
  ...
  it.skipIf(!hasBash || !hasEnvBin)(
    ...
    () => {
      ...
      const guardedScript = shellWrapperScript('/home/u/.botmux/bin', 'bash', guardDir);
