...
vi.mock('../src/im/lark/client.js', async (io) => ({ ...(await io() as object), updateMessage: (...a: any[]) => updateMessage(...a) }));
vi.mock('../src/bot-registry.js', async (io) => ({ ...(await io() as object), getBot: () => ({ config: { cliId: 'claude-code', larkAppId: 'app' }, resolvedAllowedUsers: ['ou_owner'] }), getAllBots: () => [] }));
