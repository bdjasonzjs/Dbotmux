...
describe('session.start lifecycle integration', () => {
  ...
  it('injects security env and a guard-before-wrapper PATH into an ordinary worker fork', () => {
    const guardDir = mkdtempSync(join(tmpdir(), 'botmux-normal-guard-'));
    try {
      vi.stubEnv('BOTMUX_COMMAND_GUARD', '1');
      // The daemon may be configured with a relative path even though the CLI
      // later starts in a different session cwd. The fork contract must carry
      // only the daemon-resolved absolute guard directory.
      vi.stubEnv('BOTMUX_COMMAND_GUARD_DIR', relative(process.cwd(), guardDir));
      vi.stubEnv('BOTMUX_EGRESS_PROXY', 'http://127.0.0.1:17990');

      forkWorker(makeDs(), 'hello', false);

      const forkOpts = forkMock.mock.calls.at(-1)?.[2] as { env?: Record<string, string | undefined> } | undefined;
      const env = forkOpts?.env;
      const pathParts = env?.PATH?.split(delimiter) ?? [];
      expect(env).toEqual(expect.objectContaining({
        BOTMUX_COMMAND_GUARD: '1',
        BOTMUX_PATH_PREFIX: guardDir,
        BOTMUX_EGRESS_POLICY: 'restricted-proxy',
        HTTP_PROXY: 'http://127.0.0.1:17990',
        HTTPS_PROXY: 'http://127.0.0.1:17990',
      }));
      expect(pathParts[0]).toBe(guardDir);
      expect(pathParts[1]).toBe(join(homedir(), '.botmux', 'bin'));
    } finally {
      vi.unstubAllEnvs();
  ...
  it('refuses an ordinary worker fork when an enabled guard cannot materialize', () => {
    const root = mkdtempSync(join(tmpdir(), 'botmux-normal-guard-fail-'));
    const invalidGuardPath = join(root, 'not-a-directory');
    writeFileSync(invalidGuardPath, 'occupied');
    try {
      vi.stubEnv('BOTMUX_COMMAND_GUARD', '1');
      vi.stubEnv('BOTMUX_COMMAND_GUARD_DIR', invalidGuardPath);

      expect(() => forkWorker(makeDs(), 'hello', false)).toThrow(/refusing worker start/i);
      expect(forkMock).not.toHaveBeenCalled();
    } finally {
      vi.unstubAllEnvs();
  ...
  it('injects security env and a guard-before-wrapper PATH into an adopt worker fork', () => {
    const guardDir = mkdtempSync(join(tmpdir(), 'botmux-adopt-guard-'));
    try {
      vi.stubEnv('BOTMUX_COMMAND_GUARD', '1');
      vi.stubEnv('BOTMUX_COMMAND_GUARD_DIR', relative(process.cwd(), guardDir));
      vi.stubEnv('BOTMUX_EGRESS_PROXY', 'http://127.0.0.1:17990');
  ...
  it('refuses adopt before lifecycle mutation when an enabled guard cannot materialize', () => {
    const root = mkdtempSync(join(tmpdir(), 'botmux-adopt-guard-fail-'));
    const invalidGuardPath = join(root, 'not-a-directory');
    writeFileSync(invalidGuardPath, 'occupied');
    const existingWorker = makeFakeWorker();
    const ds = makeDs({
    ...
    try {
      ...
      expect(() => forkAdoptWorker(ds)).toThrow(/refusing worker start/i);
      expect(forkMock).not.toHaveBeenCalled();
      expect(existingWorker.kill).not.toHaveBeenCalled();
      expect(ds.worker).toBe(existingWorker);
      expect(ds.workerGeneration).toBeUndefined();
    } finally {
      vi.unstubAllEnvs();
      rmSync(root, { recursive: true, force: true });
    }
