...
describe('core-only entrypoint hardening (codex 4 P1s — source lock)', () => {
  ...
  it('P1(4th round): wrapper bin dir has ONE resolver; every PATH consumer routes through it (no hardcoded ~/.botmux/bin prepend)', () => {
    ...
    expect(wpSrc).toContain('const botmuxBinDir = resolveBotmuxWrapperBinDir(base);');
    expect(wpSrc).toContain('const forkEnv = securedWorkerForkEnv(process.env);');
    const ordinaryFork = region(wpSrc, 'export function forkWorker(', 'export function forkAdoptWorker(');
    expect(ordinaryFork.indexOf('const forkEnv = securedWorkerForkEnv(process.env);'))
      .toBeLessThan(ordinaryFork.indexOf('const workerGeneration = reserveWorkerGeneration(ds);'));
    const adoptFork = region(wpSrc, 'export function forkAdoptWorker(', 'export function listProcesses(');
    expect(adoptFork.indexOf('const forkEnv = securedWorkerForkEnv(process.env);'))
      .toBeLessThan(adoptFork.indexOf('const workerGeneration = reserveWorkerGeneration(ds);'));
    expect(adoptFork.indexOf('const forkEnv = securedWorkerForkEnv(process.env);'))
      .toBeLessThan(adoptFork.indexOf('ds.workerReady = false;'));
    ...
    // scrubbed before the script runs). shellWrapperScript takes both host-resolved
    // wrapper and security PATH prefixes; call sites resolve them from opts.env. NO hardcoded
    ...
    expect(tmuxSrc).toContain('export function shellWrapperScript(');
    expect(tmuxSrc).toContain('const wrapperBinDir = resolveBotmuxWrapperBinDir(opts.env ?? process.env);');
    expect(tmuxSrc).toContain(': shellWrapperScript(wrapperBinDir, shellKind, securityPathPrefix);');
    ...
    const fishAwarePersistentBackendCalls: Record<string, RegExp> = {
      'src/adapters/backend/tmux-pipe-backend.ts': /shellWrapperScript\(\s*resolveBotmuxWrapperBinDir\(opts\.env \?\? process\.env\),\s*shellKindForPath\(shellSpec\.shell\),\s*opts\.env\?\.BOTMUX_PATH_PREFIX,\s*\)/,
      'src/adapters/backend/zellij-backend.ts': /shellWrapperScript\(\s*resolveBotmuxWrapperBinDir\(opts\.env \?\? process\.env\),\s*kind,\s*opts\.env\?\.BOTMUX_PATH_PREFIX,\s*\)/,
      'src/adapters/backend/zmx-backend.ts': /shellWrapperScript\(wrapperBinDir, shellKind, opts\.env\?\.BOTMUX_PATH_PREFIX\)/,
    };
