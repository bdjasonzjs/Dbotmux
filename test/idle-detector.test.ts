...
// ─── CoCo readyPattern variants (legacy 0.120.x + TraeCode 0.201.x) ──────
...
describe('IdleDetector: CoCo readyPattern compatibility', () => {
  ...
  it('matches the TraeCode context bar used by CoCo 0.201.x', () => {
    const detector = new IdleDetector(cocoAdapter);
    const cb = vi.fn();
    detector.onIdle(cb);
    detector.feed('GPT-5.5 · Context 100% left · /tmp');
    vi.advanceTimersByTime(2500);
    expect(cb).toHaveBeenCalledTimes(1);
    detector.dispose();
  });

  it('does not treat the workspace-trust selector as a ready composer', () => {
    const detector = new IdleDetector(cocoAdapter);
    const cb = vi.fn();
    detector.onIdle(cb);
    detector.feed('\n❯ 1. Yes, continue\n  2. No, quit');
