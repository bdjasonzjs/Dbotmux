...
const CONSUMERS: Record<string, Rule> = {
  ...
  'core/dashboard-ipc-server.ts::<module>::closeSession': {
    category: 'user_surface',
    why: 'Close routes serialise the whole result and replay closed-row residuals; '
      + 'manager recovery refuses a failed close before replacing the local worker; '
      + 'the overload sweep logs per-session failures and its aggregate outcome.',
    count: 5,
  },
