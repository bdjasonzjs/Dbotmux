...
function configuredProjectFor(dataDir: string, cwd: string): string {
  const projectHash = realpathSync(cwd).replace(/[^A-Za-z0-9-]/g, '-');
  return join(dataDir, 'projects', projectHash, `${SID}.jsonl`);
}
...
describe('syncClaudeResumeTargetToCwd', () => {
  it('copies a cwd-scoped transcript into the new cwd before resume', () => {
    ...
    mkdirSync(oldCwd);
    mkdirSync(newCwd);
    const source = configuredProjectFor(dataDir, oldCwd);
    ...
    expect(result).toEqual({
      targetPath: configuredProjectFor(dataDir, newCwd),
      sourcePath: source,
      copied: true,
    });
  ...
  it('refreshes a stale target from the newest cwd copy', () => {
    ...
    mkdirSync(newCwd);
    const source = configuredProjectFor(dataDir, oldCwd);
    const target = configuredProjectFor(dataDir, newCwd);
    mkdirSync(join(source, '..'), { recursive: true });
