...
describe('Agent Workbench visual contract', () => {
  ...
  const workbenchStart = css.indexOf('/* Agent Workbench');
  const workbenchEnd = css.indexOf('\n.toast-stack', workbenchStart);
  // Workbench used to be the final section in style.css, so slicing to EOF was
  // equivalent to selecting the component's own rules. Dashboard-wide Toast,
  // modal, overview, session-board, and legacy topology styles now follow it;
  // keep this contract scoped to the Workbench section instead of rejecting
  // unrelated global gradients and legacy pixel radii added after that boundary.
  expect(workbenchStart).toBeGreaterThanOrEqual(0);
  expect(workbenchEnd).toBeGreaterThan(workbenchStart);
  const block = css.slice(workbenchStart, workbenchEnd);
