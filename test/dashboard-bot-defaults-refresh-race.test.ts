...
// The production component intentionally portals the OAuth overlay to body so
// the page-entry transform cannot turn position:fixed into a page-local box.
// react-test-renderer has no DOM portal host; render the portal's children
// inline here so these state-machine tests can inspect the overlay without
// weakening the production full-viewport behavior.
vi.mock('react-dom', () => ({
...
describe('SessionGroupTagRow (bot-switch stale responses)', () => {
  beforeEach(() => {
    // Satisfy the production portal guard. createPortal itself is mocked above,
    // so no DOM implementation is needed for these renderer-only tests.
