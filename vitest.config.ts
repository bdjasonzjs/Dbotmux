    ...
    projects: [
      ...
      },
      {
        test: {
          name: 'e2e',
          include: ['test/**/*.e2e.ts'],
          // Feishu browser tests are an explicitly provisioned, stateful suite
          // (`pnpm test:e2e-browser` sets BOTMUX_E2E=1). Keep ordinary
          // `pnpm test:all` deterministic on hosts without login state or
          // Midscene credentials; real local-CLI/backend E2E still runs.
          exclude: process.env.BOTMUX_E2E === '1'
            ? ['node_modules/**']
