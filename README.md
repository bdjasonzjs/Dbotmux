...
botmux 不重新实现 Agent 能力，而是直接桥接已有的 AI 编程 CLI（Claude Code、Codex、Aiden、CoCo、Gemini、OpenCode）。CLI 的每次升级——新工具、更强推理、更多 MCP 支持——botmux 零适配自动受益。
...
| 安装部署 | `npm install -g botmux`，无需 Docker / Web 服务 | 需部署完整 Web 服务 + Docker |
