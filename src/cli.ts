...
/** Resolve every sender-app-scoped open_id that names one target bot app.
...
async function relaySend(
  ...
  if (cardJsonArg !== undefined || cardFile !== undefined) {
    ...
    const pos = positionals(rest, ['--card', '--text', '--footer', '--no-footer', '--top-level', '--no-quote', '--mention-back', '--no-mention', '--anyway', '--voice', '--slash']);
  ...
  const FLAGS_NOVAL = new Set(['--footer', '--no-footer', '--mention-back', '--no-mention', '--no-quote', '--voice', '--slash']);
...
async function cmdSend(rest: string[]): Promise<void> {
  ...
  // Local fork contract: footer chrome is opt-in. Explicit @ decisions remain
  // independent — a no-footer send may still render an explicitly requested @.
  ...
  if (customCardRequested) {
    const unexpectedText = positionals(rest, ['--card', '--text', '--footer', '--no-footer', '--top-level', '--no-quote', '--mention-back', '--no-mention', '--anyway', '--voice', '--attention']);
    ...
    const pos = positionals(rest, ['--card', '--text', '--footer', '--no-footer', '--top-level', '--no-quote', '--mention-back', '--no-mention', '--anyway', '--voice', '--attention', '--slash']);
    if (pos.length > 0) {
      content = pos.join(' ');
    } else {
  ...
  try {
    ...
    const frozenFooterAddressingSource = explicitVcMeetingImOrigin?.replyTargetSenderOpenId
      ...
      : frozenTurnDispatch?.replyTargetSenderOpenId
        ...
        : s;
    const footerAddressing = (sendTopLevel || noMention || noFooter)
    ...
    if (customCard) {
      ...
      const footer = buildReplyCardFooter({
        // no-footer removes only implicit chrome/addressing. Explicit --mention
        // recipients remain functional and therefore still get a minimal @ row.
        brand: noFooter ? '' : renderBrandTemplate(resolveBrandLabel(appId), s.workingDir),
...
switch (command) {
  ...
  case 'subtask-askforhelp':
  case 'subtask-request-review':
