...
export interface CliAdapter {
  ...
  /** Optional per-engine home relocation contract used by clone creation. */
  readonly cloneHome?: CloneHomeSpec;

  /** Whether this adapter can read context files written on the daemon host.
   * Undefined preserves the historical local-process default (`true`). */
  ...
  buildArgs(opts: {
    ...
    /** This session's CLI home. Adapters that inspect home-scoped state while
     *  building resume arguments must use it instead of the process-global
     *  default, otherwise cloned bots can read the primary bot's history. */
    cliHome?: string;
