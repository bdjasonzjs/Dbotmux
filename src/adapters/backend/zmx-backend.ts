...
export function buildZmxLaunchFiles(
  ...
  const wrapped = debugKeepShell
    ? buildDebugKeepShellScript(shellSpec.shell, wrapperBinDir, shellKind, opts.env?.BOTMUX_PATH_PREFIX)
    : shellWrapperScript(wrapperBinDir, shellKind, opts.env?.BOTMUX_PATH_PREFIX);
