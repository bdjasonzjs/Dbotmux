...
export function findCodexRolloutBySessionId(
  cliSessionId: string,
  homeOrOpts?: string | { codexHome?: string; noFollow?: boolean },
): string | undefined {
  // Keep the fork's positional-home API while accepting upstream's options
  // object (which adds the no-follow boundary used for isolated bot homes).
  const opts = typeof homeOrOpts === 'string'
    ? { codexHome: homeOrOpts }
    : homeOrOpts;
  ...
  for (const line of completeText.split('\n')) {
    ...
    if (obj.type === 'event_msg'
      ...
      events.push({
        ...
        timestampMs,
        kind: 'turn_aborted',
        text: typeof p.reason === 'string' && p.reason.trim()
          ? p.reason.trim()
          : 'turn_aborted',
