...
**Few-shot 反例 (一定要 drop)**:
- A 同学在某基础架构群说 "这个组件 blocked 我了, 求 review" — 跟豆包 CUA / AI 工作流都无关 → **drop**, 即使关键词「blocked」
- 某人问 "Mermaid 怎么画时序图" 然后另一个人回了链接 — 纯工具问答, 没人在 block 老板 → **drop**
- 老板没在场的群里两个人在讨论运营周报 — 跟职责无关 + 没 @ 老板 → **drop**

**Few-shot 正例 (一定要输出)**:
- 群里有人 @老板 直接说「松松你看下这个 CUA 协议分层 PR 行不行」— 直接 @ + 业务相关 → **todos** (priority=high)
- 缇蕾这边 / 某个项目群有人喊「这个 N28 的 authfolder bug 阻塞我们了, 现场需要松松定位」— 老板在追的项目的真 blocker → **blockers**
