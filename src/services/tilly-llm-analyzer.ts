...
// 注入 (那个一直好用)。coco-session-store 留着但不再被本模块使用。
...
**身份硬规则 (升级给主克劳德 CEO 的判读边界)**:
- @松松 / @Jason 只代表源群在找松松本人, **不等于** @主克劳德 CEO; 不能仅因为消息 @了松松就输出 high-prio 给主话题。
- @主克劳德 CEO 必须以真实 open_id 命中: \`ou_65c655b50c0de2f60640960bac0d9112\`。
- 如果某群已经有常驻 Claude 分身 active session, 该群里 @克劳德 的消息由本群分身处理, 不升级到主话题；除非消息明确 @的是上面的主克劳德 CEO open_id。
- 这些身份判断以 message.mentions / <at user_id="..."> 的 open_id 为准, 不按显示名或 summary 猜。
...
// 主克劳德 CEO 的 bot open_id — scout 上报主话题只认这个真实 mention。
const MAIN_CLAUDE_CEO_OPEN_ID = 'ou_65c655b50c0de2f60640960bac0d9112';
...
const CLAUDE_MENTION_NAME = /克劳德|claude/i;

/** G1: 给一条消息算 priority score（越高越优先）。同分按 createTime 倒序 tie-break。 */
function scoreMessage(m: TillyMessage): number {
  let s = 0;
  const ids = mentionedOpenIds(m);
  // +20: 真 @主克劳德 CEO 必须优先进入 prompt，避免被 @松松 噪音挤掉。
  if (ids.has(MAIN_CLAUDE_CEO_OPEN_ID)) s += 20;
...
export interface AnalyzeOpts {
  ...
  memoryToday?: string;
  /** Main Claude CEO app id. When set, active sessions owned by this app do
   *  not count as "local resident Claude clone/session handles this chat". */
  mainClaudeCeoAppId?: string;
  /** Test hook / production override: active sessions for a chat. Defaults to
   *  session-store cross-file lookup. Used only for deterministic identity
   *  filtering after the LLM returns. */
  activeSessionsForChat?: (chatId: string) => ActiveSessionSnapshot[];
}
...
function atUserIdsFromContent(content: string): string[] {
  const out: string[] = [];
  for (const m of content.matchAll(/<at\b[^>]*\buser_id=["']([^"']+)["'][^>]*>/g)) {
    out.push(m[1]);
  }
  return out;
}

function mentionedOpenIds(m: TillyMessage): Set<string> {
  const ids = new Set<string>(atUserIdsFromContent(m.content));
  for (const mention of m.mentions ?? []) {
    if (mention.openId) ids.add(mention.openId);
  }
  return ids;
}

function mentionsClaudeName(m: TillyMessage): boolean {
  if (CLAUDE_MENTION_NAME.test(m.content)) return true;
  return (m.mentions ?? []).some(mention => CLAUDE_MENTION_NAME.test(mention.name ?? ''));
}

function hasActiveClaudeSessionInChat(
  chatId: string,
  activeSessionsForChat: NonNullable<AnalyzeOpts['activeSessionsForChat']>,
  mainClaudeCeoAppId?: string,
): boolean {
  return activeSessionsForChat(chatId).some(s =>
    (s.status === undefined || s.status === 'active')
      && s.cliId === 'claude-code'
      && (!!s.larkAppId ? s.larkAppId !== mainClaudeCeoAppId : true),
  );
}

function shouldKeepDigestItem(
  item: { sourceMessageId: string },
  byId: Map<string, TillyMessage>,
  activeSessionsForChat: NonNullable<AnalyzeOpts['activeSessionsForChat']>,
  mainClaudeCeoAppId?: string,
): boolean {
  const source = byId.get(item.sourceMessageId);
  if (!source) return true;
  const ids = mentionedOpenIds(source);
  const mentionsMainClaude = ids.has(MAIN_CLAUDE_CEO_OPEN_ID);

  // @松松 是源群通知松松本人，不是 @主克劳德 CEO。避免把人的 ping
  // 误升级成主话题 @Claude high-prio。
  if (ids.has(SONGSONG_OPEN_ID) && !mentionsMainClaude) {
    logger.info(`[tilly-llm-analyzer] dropped scout item ${item.sourceMessageId}: @SongSong is not @main-Claude-CEO`);
    return false;
  }

  // 群内已有常驻 Claude 分身时，@克劳德 留给本群 session。该 hard drop
  // 先于主 CEO mention 补偿，避免常驻分身群被重新升级到主话题。
  if (mentionsClaudeName(source)
      && hasActiveClaudeSessionInChat(source.chatId, activeSessionsForChat, mainClaudeCeoAppId)) {
    logger.info(`[tilly-llm-analyzer] dropped scout item ${item.sourceMessageId}: active Claude session in chat ${source.chatId}`);
    return false;
  }
  return true;
}

function summarizeForDeterministicTodo(m: TillyMessage): string {
  const text = m.content
    .replace(/<at\b[^>]*>[^<]*<\/at>/g, '')
    .replace(/\s+/g, ' ')
    .trim();
  const suffix = text ? `：${text.slice(0, 60)}` : '';
  return `源群明确 @主克劳德 CEO，需要主话题处理${suffix}`;
}

function deterministicMainClaudeTodos(
  ...
  const out: TillyDigestItem[] = [];
  for (const m of messages) {
    if (!includedIdSet.has(m.messageId) || alreadyReportedIds.has(m.messageId)) continue;
    if (!mentionedOpenIds(m).has(MAIN_CLAUDE_CEO_OPEN_ID)) continue;
    if (!shouldKeepDigestItem({ sourceMessageId: m.messageId }, byId, activeSessionsForChat, mainClaudeCeoAppId)) continue;
    out.push({
      summary: summarizeForDeterministicTodo(m),
      sourceChatId: m.chatId,
      sourceChatName: m.chatName || m.chatId,
      sourceMessageId: m.messageId,
      priority: 'high',
      sourceAppLink: m.appLink,
    });
  }
...
export async function analyzeMessages(
  ...
  // 妹妹 P1 (2026-05-28): 总 prompt budget — MAX_PROMPT_CHARS 只约束 message
  ...
  const assemblePrefix = (mem: string) => PROMPT_PREFIX
    ...
    .replace('<MEMORY_TODAY_PLACEHOLDER />', mem)
    .replace('<KNOWN_HANDLED_TOPICS_PLACEHOLDER />', knownBlock);
  let prefix = assemblePrefix(memoryBlock);
  let prompt = prefix + renderedMessages + PROMPT_SUFFIX;
  // 只在 caller 没显式提供 memoryToday 时才退档 (test 路径不动)
  if (!opts.memoryToday) {
    for (const cap of [40, 20, 10]) {
      if (prompt.length <= MAX_TOTAL_PROMPT_CHARS) break;
      memoryCap = cap;
      memoryBlock = buildMemoryTodayBlock({ perCatHardCap: memoryCap });
      prefix = assemblePrefix(memoryBlock);
      prompt = prefix + renderedMessages + PROMPT_SUFFIX;
      logger.warn(`[tilly-llm-analyzer] prompt over budget, MEMORY_TODAY shrunk to cap=${cap}, prompt now ${prompt.length}c`);
    }
  }
  ...
  // 之前在 parse 前 save → 如果 LLM 输出不是合法 JSON, 坏 session 固化,
  // 下 tick 继续 resume 同一坏 session 反复失败。
  let parsed: any;
  try {
    try {
  ...
  const activeSessionsForChat = opts.activeSessionsForChat ?? findActiveSessionsByChatId;
  const mainClaudeCeoAppId = opts.mainClaudeCeoAppId ?? process.env.BOTMUX_TILLY_MAIN_CLAUDE_CEO_APP_ID;
  ...
  const enrich = (it: any): TillyDigestItem | null => {
    ...
    if (!shouldKeepDigestItem(it, byId, activeSessionsForChat, mainClaudeCeoAppId)) return null;
  ...
  const todos = safe(parsed.todos);
  const progress = safe(parsed.progress);
  const blockers = safe(parsed.blockers);
  const noteworthy = safe(parsed.noteworthy);
  ...
  const mainClaudeTodos = deterministicMainClaudeTodos(
    messages,
    includedIdSet,
    alreadyReportedIds,
    byId,
    activeSessionsForChat,
    mainClaudeCeoAppId,
  );

  const result: TillyDigest = {
    todos: [...mainClaudeTodos, ...todos].slice(0, 5),
    progress,
    blockers,
    noteworthy,
    inputMessageCount: messages.length,
