...
import {
  DEFAULT_GROUP_NOT_FOUND_RETRY_TIMEOUT_MS,
  writeRelayRecord, checkRelayStatus,
} from './base-relay.js';
...
function parentToChildSummonBody(cmd: OutboxCommand, task: SubTask, writeLetterFn: WriteLetterFn): string {
  if (cmd.commandType === 'kickoff') {
    ...
    const kickoffTargets = cmd.payload.targetSummonName ? [cmd.payload.targetSummonName] : resolveTargets(task, 'main');
    ...
    const buildKickoff = (goalSlot: string, accSlot: string): string => {
      ...
      return urgentSummon(kickoffTargets,
...
export function makeDispatchExecutors(): DispatchExecutors {
  return {
    ...
    async writeAndSend(cmd: OutboxCommand, task: SubTask) {
      ...
      // v2 非阻塞：只写 record 触发自动化、**不在此轮询**。「已发送」确认由 Phase B 对账
      // (checkStatus → checkRelayStatus) 异步完成，所以这里不传 pollTimeoutMs（writeRelayRecord 本就不轮询）。
