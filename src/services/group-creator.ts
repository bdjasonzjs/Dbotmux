...
export interface CreateGroupOpts {
  ...
  /** Parent chat for topology-aware group creation. */
  sourceChatId?: string | null;
  /** Optional purpose shown in the first group context card. */
  purpose?: string;
  /** Rich context passed to the first chat-created dispatch. */
  chatContext?: {
...
export async function createGroupWithBots(opts: CreateGroupOpts): Promise<CreateGroupResult> {
  ...
  try {
    ...
    chatContextError = error instanceof Error ? error.message : String(error);
    logger.error(`[group-creator] dispatchChatCreated failed for chat ${r.chatId}: ${chatContextError}`);
  }
