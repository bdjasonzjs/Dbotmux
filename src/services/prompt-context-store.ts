...
/**
 * Remove an unclaimed sidecar for one authoritative turn. A producer can
 * legitimately fall back to inline delivery after an earlier preparation of
 * the same turn (for example when prompt-hook preflight changes). Leaving that
 * sidecar behind would let the hook inject the envelope a second time.
 */
export function removePromptContextTurn(sessionId: string, turnId: string): void {
  try {
    const dir = sessionDir(sessionId);
    ...
    for (const name of readdirSync(dir)) {
      ...
      try {
        ...
        if (parsed?.turnId === turnId) unlinkSync(file);
      } catch { /* damaged/unreadable entries remain subject to normal pruning */ }
    }
  } catch { /* best-effort */ }
}
