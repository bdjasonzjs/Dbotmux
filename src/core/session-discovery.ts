...
export interface AdoptableSession {
  ...
  /** Adopted process's own CLAUDE_CONFIG_DIR, when discoverable. */
  claudeConfigDir?: string;
}

/** Parse one process environment blob without trusting shell expansion. */
...
function readPidClaudeConfigDir(pid: number): string | undefined {
  ...
  try {
    return parseClaudeConfigDirFromEnviron(readFileSync(`/proc/${pid}/environ`, 'utf8'));
  } catch {
    return undefined;
  }
}
