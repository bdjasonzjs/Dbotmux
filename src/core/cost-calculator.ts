...
export function getSessionJsonlPath(sessionId: string, cwd: string, claudeHome?: string): string | null {
  if (claudeHome) {
    const projectKey = resolve(expandHome(cwd)).replace(/\//g, '-');
    const explicitPath = join(claudeHome, 'projects', projectKey, `${sessionId}.jsonl`);
