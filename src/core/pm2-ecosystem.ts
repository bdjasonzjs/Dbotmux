...
export function buildEcosystemConfig(bots: any[], paths: EcosystemPaths): { apps: any[] } {
  ...
  apps.push({
    ...
    merge_logs: true,
    env: {
      // Belt-and-suspenders for the topology-empty-after-restart bug (Bug A): pin
      // the dashboard's data dir to the same place the daemons write (dataDir =
      // ~/.botmux/data). config.ts now also defaults here, so this is a defensive
      // duplicate — if someone reverts the config default, the dashboard still
      // resolves the daemon's data dir instead of the empty install-dir/data.
      SESSION_DATA_DIR: paths.dataDir,
