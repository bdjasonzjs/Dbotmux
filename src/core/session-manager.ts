...
// the upstream ContextProvider registry.
export {
  buildChatContextBlock,
  buildMainBotPromptBlock,
  buildOutputDisciplineBlock,
  buildSubtaskMemberBlock,
...
function registeredBotOpenId(larkAppId: string | undefined): string | undefined {
  if (!larkAppId) return undefined;
  try {
    return getBot(larkAppId).botOpenId;
  } catch {
    // Prompt builders are also used by one-shot/offline paths where the bot is
    // not registered in this process. Mention history is optional context.
    return undefined;
  }
}
...
export function buildNewTopicPrompt(
  ...
  legacyLarkAppId?: string,
  legacyAmbientContextBlock?: string,
  legacySelfMentionedThisTurn?: boolean,
): string {
  // Keep the fork's pre-upstream positional context-delivery call shape working
  // while upstream callers use the consolidated options object.
  const opts = typeof optsOrChatId === 'string'
    ? {
        chatId: optsOrChatId,
        larkAppId: legacyLarkAppId,
        ambientContextBlock: legacyAmbientContextBlock,
        selfMentionedThisTurn: legacySelfMentionedThisTurn,
      }
    : optsOrChatId === undefined && (
        legacyLarkAppId !== undefined
        || legacyAmbientContextBlock !== undefined
        || legacySelfMentionedThisTurn !== undefined
      )
      ? {
          larkAppId: legacyLarkAppId,
          ambientContextBlock: legacyAmbientContextBlock,
  ...
  if (opts?.ambientContextBlock?.trim()) {
    parts.push(opts.ambientContextBlock);
  }

  // 半静态上下文块（chat_context / main_bot_routing / subtask_member_routing /
  // output_discipline）：走 ContextProvider registry 装配。inline 模式（默认）下
  // 输出与历史逐字节一致；file 模式下物化进本地文件、这里只插一个 <context_ref> stub。
  parts.push(...buildContextBlocks({
    chatId: opts?.chatId,
  ...
  const selfOpenIdForMentions = botIdentity?.openId
    ?? registeredBotOpenId(opts?.larkAppId);
  const recentMentionsBlock = buildRecentMentionsBlock(
    opts?.larkAppId,
    opts?.chatId,
    selfOpenIdForMentions,
    mentions,
    Date.now(),
    opts?.selfMentionedThisTurn,
  );
 ...
 */
type FollowUpBlockKey = 'sessionId' | 'role' | 'summaryMemory' | 'context' | 'recentMentions' | 'reminder' | 'whiteboard' | 'userMessage' | 'sender' | 'substitute' | 'senderNote' | 'attachments' | 'mentions';
...
type FollowUpOpts = {
  ...
  larkAppId?: string;
  chatId?: string;
  /** Whether this bot was authoritatively mentioned on this turn. */
  selfMentionedThisTurn?: boolean;
  whiteboardId?: string;
  substituteTrigger?: SubstituteTrigger;
  codexAppText?: string;
...
function buildFollowUpBlocks(
  ...
  for (const text of buildContextBlocks({
    chatId: opts?.chatId,
    larkAppId: opts?.larkAppId,
    round: 'follow_up',
    cliId: opts?.cliId,
    cliPathOverride: opts?.cliPathOverride,
    isAdoptMode: opts?.isAdoptMode,
  })) {
    blocks.push({ key: 'context', text });
  }
  const selfOpenIdForMentions = registeredBotOpenId(opts?.larkAppId);
  const recentMentionsBlock = buildRecentMentionsBlock(
    opts?.larkAppId,
    opts?.chatId,
    selfOpenIdForMentions,
  ...
  if (recentMentionsBlock) blocks.push({ key: 'recentMentions', text: recentMentionsBlock });
  if (opts?.cliId !== 'mira') {
...
export function buildFollowUpCliInput(
  ...
  if (hookTurnId) removePromptContextTurn(sessionId, hookTurnId);
...
// Several isolated consumers intentionally provide a partial config object.
// Keep the restore defaults available there instead of crashing at import time.
...
export async function restoreActiveSessions(
  ...
  for (const session of active) {
    ...
    if (isTillyMainTopicConversationDenied(session.cliId, session.chatId)) {
      logger.info(
        `[${session.larkAppId ?? 'unknown'}] closing forbidden coco mainTopic session during restore `
...
| { ok: false; error: 'not_found' | 'not_closed' | 'anchor_occupied' | 'adopt_unsupported' | 'vc_receiver_managed' | 'deferred_unmaterialized' | 'resume_cancelled' | 'conversation_denied'; activeSessionId?: string }> {
