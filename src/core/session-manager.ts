...
import { buildContextBlocks, xmlEscape } from './context-providers.js';
import { recordMention, getRecentMentions, MAX_RECENT_MENTIONS } from '../services/mention-history-store.js';

// 半静态上下文块（chat_context / main_bot_routing / subtask_member_routing / output_discipline）
// 已收敛进 ContextProvider registry（./context-providers.ts），这里 re-export 保持既有 API/测试面不变。
export { buildChatContextBlock, buildMainBotPromptBlock, buildOutputDisciplineBlock, buildSubtaskMemberBlock } from './context-providers.js';
...
export function buildNewTopicPrompt(
  ...
  parts.push(...buildContextBlocks({ chatId, larkAppId, round: 'new_topic', cliId, cliPathOverride }));
...
export function buildFollowUpContent(
  ...
  // 半静态上下文块（main_bot_routing / subtask_member_routing / output_discipline；
  // chat_context 历史上只在首轮 inline 注入）：走 ContextProvider registry 装配，
  // 语义同 buildNewTopicPrompt 处的注释。多轮对话会丢这些信息，每条后续消息都补。
