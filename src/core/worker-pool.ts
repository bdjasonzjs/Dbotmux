...
import { resolveClaudeHomeForAdopt } from './claude-home.js';
import {
  buildMarkdownCard,
...
/**
 * Build the daemon -> worker launch environment shared by ordinary and adopt
 * forks.  Security has to be produced at this boundary: the worker/backend can
 * only consume BOTMUX_PATH_PREFIX and proxy-policy keys that the daemon places
 * in its fork environment.
 *
 * PATH order is deliberate: command guards must intercept before the botmux
 * wrapper, and both must precede the inherited host PATH.
 */
function securedWorkerForkEnv(base: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const securityEnv = resolveWorkerSecurityEnv(base);
  const guardDir = ensureCommandGuardShims({ ...base, ...securityEnv });
  if (securityEnv.BOTMUX_COMMAND_GUARD === '1' && !guardDir) {
    throw new Error('command guard is enabled but no materialized guard directory was returned');
  }
  const botmuxBinDir = resolveBotmuxWrapperBinDir(base);
  const pathWithBotmux = prependBotmuxBin(botmuxBinDir, base.PATH);
  const securedPath = guardDir
    ? prependBotmuxBin(guardDir, pathWithBotmux)
    : pathWithBotmux;
...
export interface WorkerPoolCallbacks {
  ...
  /** Apply queued-input metadata only when the worker actually starts it. */
...
/** Process-liveness probe used by the fork's session sweeper. An attached
 * worker handle is authoritative (and immune to PID reuse); only workerless
 * rows fall back to signal-0 probing of the persisted pid. */
export function isSessionLive(ds: DaemonSession): boolean {
  const worker = ds.worker;
  if (worker) {
    return !worker.killed && worker.exitCode === null && worker.signalCode === null;
  }
...
export function forkWorker(
  ...
  // Fail closed before mutating session state, reserving a generation, or
  // retiring an existing worker. An enabled guard that cannot materialize its
  // complete PATH interception layer must never fall through to an unguarded
  // child fork.
  const forkEnv = securedWorkerForkEnv(process.env);
...
export function forkAdoptWorker(ds: DaemonSession, opts?: { restoredFromMetadata?: boolean; prompt?: string; turnId?: string }): void {
  ...
  // Prepare the same fail-closed security boundary as ordinary workers before
  // reserving a generation or killing the currently attached bridge worker.
  // If materialization throws, no child fork and no lifecycle mutation occurs.
  const forkEnv = securedWorkerForkEnv(process.env);
  ds.workerReady = false;
  ...
  const worker = fork(workerPath, [], {
    ...
    env: {
      ...
      CLAUDECODE: undefined,
      BOTMUX: '1',
      // Adopt workers still host botmux-aware bridge/skill subprocesses. Keep
      // their routing bound to the adopted daemon session exactly like the
      // ordinary worker path; otherwise botmux send can fall back to a stale
      // inherited shell session.
      BOTMUX_SESSION_ID: ds.session.sessionId,
      LARK_APP_ID: botCfg.larkAppId,
