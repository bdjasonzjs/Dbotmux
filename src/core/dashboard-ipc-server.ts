...
import { AskHumanIpcEndpoint, ASK_HUMAN_IPC_ROUTE, ASK_HUMAN_IPC_MAX_BYTES, type AskHumanIpcHandler } from './ask-human-ipc.js';
...
import { buildNewTopicPrompt, rememberLastCliInput, resumeSession, spawnDashboardSession, activateQueuedSession, closeCliMismatchedSessionsForBot } from './session-manager.js';
...
import { sessionAnchorId, sessionKey, larkTransportEnabled, type DaemonSession } from './types.js';
...
export function setAskHumanIpcApi(api: AskHumanIpcHandler | null): void { askHumanEndpoint.install(api); }
...
ipcRoute('POST', ASK_HUMAN_IPC_ROUTE, async (req, res) => {
  ...
  const outcome = await askHumanEndpoint.handle(body, { trustedHost: isTrustedHostIpcRequest(req) });
...
function routeHasNarrowUntrustedAuth(method: string, pathname: string): boolean {
  ...
  // Legacy operations verify a rotating capability in AskHumanApi. The
  // independent report operation requires the verified local host HMAC.
...
ipcRoute('POST', '/api/sessions/:sessionId/manager-recover', async (req, res, params) => {
  ...
  const closed = await closeSession(old.session.sessionId);
  if (!closed.ok) {
    return jsonRes(res, 409, { ok: false, error: 'close_failed', detail: closed });
  }
  // A residual means the local row closed but a remote agent or an unproven
  // host subtree may still be running with the old credential boundary. Never
  // start its replacement: doing so can duplicate the task and overlap two
  // credential-bearing executors. Surface the exact residual for operator
  // cleanup and fail closed.
  if (closed.outcome === 'closed_with_residual') {
    return jsonRes(res, 409, { ok: false, error: 'close_residual', detail: closed });
  }
  reg.delete(oldKey);
  ...
  const cliInput = buildNewTopicPrompt(
    ...
    undefined,
    { chatId: session.chatId, larkAppId: old.larkAppId },
  );
