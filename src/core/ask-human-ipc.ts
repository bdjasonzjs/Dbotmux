...
export interface AskHumanIpcHandler {
  handle(input: unknown, context?: { trustedHost: boolean }): Promise<unknown>;
}
...
export class AskHumanIpcEndpoint {
  private api: AskHumanIpcHandler | null = null;
  install(api: AskHumanIpcHandler | null): void { this.api = api; }
  async handle(input: unknown, context?: { trustedHost: boolean }): Promise<AskHumanIpcResult> {
    const api = this.api;
    if (!api) return { status: 503, body: { ok: false, error: 'NOT_ENABLED' } };
    try {
      return { status: 200, body: { ok: true, result: (await api.handle(input, context)) ?? null } };
    } catch (error) {
