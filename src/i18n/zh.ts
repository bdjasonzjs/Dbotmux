...
export const messages: Record<string, string> = {
  ...
  'card.model.title': '⚙ 模型 · {cliName}',
  'card.model.current': '当前运行：{model}',
  'card.model.cli_default': 'CLI 默认',
  'card.model.unverified': '未知（尚未核验）',
  'card.model.pinned': '本会话意愿：{model}',
  'card.model.switching': '🔄 正在切换到 {target}…（切换中不接受新的切换）',
  'card.model.ambiguous': '⚠️ 上次切换到 {target} 超时、结果未知，已冻结切换。请「复核」或「强制回滚」。',
  'card.model.fresh_thread_note': 'ℹ️ 该 CLI 切模型会开启新线程（旧线程历史不带入）。',
  'card.model.candidates_live': '可切换的模型（实时探测）：',
  'card.model.candidates_static': '可切换的模型（内置候选）：',
  'card.model.no_candidates': '该 CLI 没有可枚举的候选，请用「自定义」输入模型名。',
  'card.model.effort_title': '推理强度：',
  'card.model.btn_custom': '✏️ 自定义',
  'card.model.btn_refresh': '🔄 刷新候选',
  'card.model.btn_recheck': '🔍 复核',
  'card.model.btn_force_rollback': '↩ 强制回滚',
  'card.model.btn_switch': '切换',
  'card.model.btn_confirm_switch': '确认切换（会中断当前回合）',
  'card.model.custom_title': '✏️ 自定义模型 · {cliName}',
  'card.model.custom_note': '输入模型名后点「切换」。留空 = 使用 CLI 默认模型。名字不合法时 CLI 会启动失败并自动回滚。',
  'card.model.custom_placeholder': '例如 opus / gpt-5.5 / deepseek/deepseek-v4-pro',
  'card.model.confirm_title': '⚠️ 确认切换 · {cliName}',
  'card.model.confirm_busy': '{cliName} 正在处理消息。切换到 **{target}** 会重启 CLI 并中断当前回合。',
  'card.model.switch_started': '🔄 正在切换 {cliName} 到 {target}…',
  'card.model.switch_succeeded': '✅ {cliName} 已切换到 {target}。',
  'card.model.switch_failed': '❌ {cliName} 切换到 {target} 失败，已回滚到 {previous}。',
  'card.model.switch_ambiguous': '⌛ {cliName} 切换到 {target} 超时、结果未知。已冻结切换，请在模型菜单「复核」或「强制回滚」。',
  'card.model.effort_dropped': 'ℹ️ 原强度 {effort} 在 {model} 下不可用，已回落 CLI 默认档。',
  'card.model.refuse.unsupported': '该 CLI 不支持切换模型。',
  'card.model.refuse.switch_in_flight': '正在切换中，请等待结果。',
  'card.model.refuse.restart_in_flight': '正在重启中，请等待重启完成再切换。',
  'card.model.refuse.ambiguous_frozen': '上次切换结果未知，已冻结。请先「复核」或「强制回滚」。',
  'card.model.refuse.effort_not_supported': '该模型不支持这个推理强度。',
  'card.model.refuse.effort_not_configurable': '该 CLI 不支持设置推理强度。',
  'card.model.refuse.transferring': '会话正在迁移，请稍后再试。',
  'card.model.refuse.remote': '远程后端会话不支持切换模型。',
  'card.model.refuse.adopt': 'adopt 模式会话不支持切换模型。',
  'card.model.refuse.external': '仅限已确认为内部群/私聊的会话使用。',
  'card.model.refuse.same': '已经是这个模型 / 强度了。',
  'card.model.refuse.no_txn': '当前没有待复核的切换。',
  'card.model.recheck_committed': '✅ 复核通过：{cliName} 正在运行 {target}。',
  'card.model.recheck_rolled_back': '↩ 复核发现 {cliName} 仍在运行旧模型，已回滚记录。',
  'card.model.recheck_ambiguous': '❓ 仍无法核实当前模型（没有新一代的启动核验）。可「强制回滚」或稍后再复核。',
  'card.model.invalid_model': '模型名不合法（只允许字母数字和 . _ - / :）。',
  'card.model.confirm_fresh': '{cliName} 切换模型会**开启新线程**，当前线程的对话历史不会带入 **{target}**。确认后才会执行。',
  ...
  'card.model.convergence_refused': '⚠️ 进程未能自动收敛到旧模型（有其它重启在飞或会话迁移中），请稍后手动 /restart。',
  ...
  'card.btn.model_switch': '🎛 选择模型',
  'card.model.btn_exit_select': '↩ 退出选择',
  'card.model.btn_switching': '⏳ 切换中…',
  'card.model.btn_confirm': '确认切换',
  'card.model.btn_cancel': '取消',
  'card.model.restart_in_flight_note': '⏳ 有一次重启正在进行，完成后才能切换（按钮已置灰）。',
  'card.model.confirm_plain_inline': '切到 **{target}**？会重启 CLI（本会话历史保留），完成后自动继续当前任务。',
  'card.model.confirm_busy_inline': '切到 **{target}**？CLI 正在处理消息，切换会重启 CLI 并中断当前回合；完成后自动补发「继续」。',
  'card.model.confirm_fresh_inline': '切到 **{target}**？该 CLI 切模型会**开启新线程**（旧线程历史不带入）；完成后自动补发「继续」。',
  'card.model.failed_inline': '❌ 切换到 {target} 失败：{reason}。已回滚。',
  'card.model.auto_continue_text': '继续',
  'card.model.reason_restart_failed': '{cliName} 用新模型重启失败',
  'card.model.custom_not_in_v2': '当前只支持从列表选择模型。',
