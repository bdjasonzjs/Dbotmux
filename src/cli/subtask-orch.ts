...
const LIST_FLAGS = new Set(['bots', 'sourceMessageIds', 'relatedRefs', 'evidenceLinks']);
...
const HELP = `botmux subtask-{start|report|query|finish|supplement} — 子任务编排 v2
  ...
  subtask-request-review --task-id <id> --summary "<可打开的飞书链接/本机绝对路径>"
                     [--evidence-links url1,url2] [--source-message-ids m1,m2]
                     (执行者产出后唤起 reviewer；只能从子群、由执行者(main)调；summary 必须含可打开链接/绝对路径)
                     (require-evidence：需用 --evidence-links 附验收证据，否则 service 层 422)
