...
/** Fork security controls that must be delivered together. The proxy policy
 * is fail-closed only when the guard-bin prefix and command guard survive the
 * same pane boundary, so keep the four keys as one audited transport unit. */
export const SECURITY_GUARD_ENV_KEYS = [
